    ...
    <x-backend.section-header>
        <div class="d-flex flex-wrap gap-3">
            ...
            @if(collect($permissionsToCheck)->contains(fn ($permission) => auth()->user()->can($permission)))
                <x-backend.quick-action url="{{ route('backend.medicine-form.bulk_action') }}">
                    <div class="">
                        <select name="action_type" class="form-control select2 col-12" id="quick-action-type" style="width:100%">
                            <option value="">{{ __('messages.no_action') }}</option>
                            @hasPermission('edit_medicine_form')
        ...
        <x-slot name="toolbar">
            ...
            <div class="input-group flex-nowrap border rounded">
             ...
             @if (auth()->user()->hasAnyRole(['admin', 'demo_admin']) || (auth()->user()->hasRole('pharma') && auth()->user()->can('add_medicine_form')))
                    <a href="{{ route('backend.medicine-form.create') }}" class="btn btn-primary">
...
<script type="text/javascript" defer>
    ...
    document.addEventListener('DOMContentLoaded', (event) => {
        ...
        initDatatable({
            url: '{{ route("backend.medicine-form.index_data") }}',
            finalColumns,
            advanceFilter: () => {},
            orderColumn: [
                [1, "asc"]
