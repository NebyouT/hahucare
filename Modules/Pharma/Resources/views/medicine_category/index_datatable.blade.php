    ...
    <div class="table-content mb-3">
        <x-backend.section-header>
            <div class="d-flex flex-wrap gap-3">
                @php
                    $permissionsToCheck = ['edit_medicine_category', 'delete_medicine_category'];
                @endphp

                @if (collect($permissionsToCheck)->contains(fn($permission) => auth()->user()->can($permission)))
                    <x-backend.quick-action url="{{ route('backend.medicine-category.bulk_action') }}">
                        <div class="">
                            <select name="action_type" class="form-control select2 col-12" id="quick-action-type"
                                style="width:100%">
                                <option value="">{{ __('messages.no_action') }}</option>
                                @hasPermission('edit_medicine_category')
            ...
            <x-slot name="toolbar">
                ...
                @if (auth()->user()->hasAnyRole(['admin', 'demo_admin']) ||
                        (auth()->user()->hasRole('pharma') && auth()->user()->can('add_medicine_category')))
                    <a href="{{ route('backend.medicine-category.create') }}" class="btn btn-primary ms-3">
...
@push('after-scripts')
    ...
    <script type="text/javascript" defer>
        ...
        document.addEventListener('DOMContentLoaded', (event) => {
            ...
            initDatatable({
                url: '{{ route('backend.medicine-category.index_data') }}',
                finalColumns,
                advanceFilter: () => {},
                orderColumn: [
                    [1, "asc"]
