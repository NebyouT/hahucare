...
class SupplierTypeExport implements FromCollection, WithHeadings, WithEvents, WithCustomStartCell
{
        ...
        foreach ($this->columns as $column) {
            switch ($column) {
                case 'name':
                    ...
                    break;
                case 'description':
                    $headings[] = __('messages.description');
                    break;
                case 'status':
    ...
    public function collection()
    {
        ...
        return $supplierTypes->map(function ($supplierType) {
            $data = [];
            foreach ($this->columns as $column) {
                switch ($column) {
                    case 'name':
                        ...
                        break;
                    case 'description':
                        $data[] = $supplierType->description ? $supplierType->description : __('pharma::messages.no_description');
                        break;
                    case 'status':
                        $data[] = $supplierType->status ? 'Active' : 'Inactive';
                        break;
                    default:
                        $data[] = $supplierType->{$column} ?? '-';
                        break;
                }
            }
            return $data;
