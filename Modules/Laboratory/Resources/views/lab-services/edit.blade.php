...
@section('title', 'Edit Lab Service')
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="mb-0">Edit Lab Service</h4>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('backend.lab-services.update', $service->id) }}">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('lab_id') is-invalid @enderror" 
                                        ...
                                        @foreach($labs as $lab)
                                            <option value="{{ $lab->id }}" 
                                                    {{ old('lab_id', $service->lab_id) == $lab->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                                <small class="text-muted">{{ Str::length($service->description) }} characters</small>
                        </div>

                        <div class="d-flex justify-content-between">
                            <a href="{{ route('backend.lab-services.index') }}" class="btn btn-secondary">
...
$(document).ready(function() {
    ...
    $('#name').on('input', function() {
        ...
        var slug = name.toLowerCase()
                       ...
                       .replace(/[\s_-]+/g, '-')
                       .replace(/^-+|-+$/g, '');
        // Slug is generated automatically in the controller, so we don't need to display it
    });
});
</script>
@endpush
