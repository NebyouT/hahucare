...
// dd($userResource);
    ...
    public function login(LoginRequest $request)
    {
        ...
        if ($usertype == "doctor"  || $usertype == "vendor" || $usertype == "receptionist") {
            ...
            if (multiVendor() == "0") {
                ...
                $doctorVendorType = optional(optional($user->doctor)->vendor)->user_type;
                $receptionistVendorType = optional(optional($user->receptionist)->vendor)->user_type;

                if (
                    $usertype == 'vendor' ||
                    ($usertype == 'doctor' && $doctorVendorType === 'vendor') ||
                    ($usertype == 'receptionist' && $receptionistVendorType === 'vendor')
                ) {
    ...
    public function userDetails(Request $request)
    {
        ...
        if (
            ...
            ($roleNames && $roleNames->contains('pharma'))
        ) {
            $user_commission = EmployeeCommission::where('employee_id', $user->id)->with('mainCommission')->get();
            $user_commission_data = [];
            foreach ($user_commission as $commission) {
                $user_commission_data[] = [
                    'name' => $commission->mainCommission->title ?? '',
                    'type' => $commission->mainCommission->commission_type ?? '',
                    'value' => (float)$commission->mainCommission->commission_value,
                ];
            }
            $user['commissions'] = $user_commission_data;
        }

        return response()->json([
            'status' => true,
            'data' => $user,
            'message' => __('messages.user_details_successfull')
        ]);
    }
